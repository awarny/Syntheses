...
Nous parlons d'une équation du type : $y' = f(x).g(y)$. dont la solution est : $$\forall x \in I, \frac{d \phi}{dx}(x) = f(x).g(\phi (x))$$
...
Méthode pratique, \textit{cf. Analyse I, 6.7}.
...
où $D$ est une droite passant par $\vec{a}$.\\
...
Parfois, selon qu'on tende par un axe et puis par un autre, les dérivées peuvent être différentes : discordance des dérivées partielles.\\
